...
var player = new Carrot();
internals.scene.add(player.mesh);
...
function cubeUpdate(){
	...
	// gravity effect
	player.mesh.position.y -= Math.ceil(deltaTime*cubeSpeedY+g*deltaTime*deltaTime/2);
	cubeSpeedY += g*deltaTime;
	// if cube touch the floor
	if(player.mesh.position.y < -fieldHeight/2+cubeSize/2){
		//gameOverFun();
		cubeDied = true;
		player.mesh.position.y = -fieldHeight/2+cubeSize/2;
	}
